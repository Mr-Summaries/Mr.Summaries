...
export const Layout = () => {
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-zinc-950 text-zinc-50">טוען...</div>;
  }

  return (
    <div className="min-h-screen text-zinc-50 transition-colors duration-300 relative flex flex-col" dir="rtl">
      <InteractiveBackground />
      <DisclaimerModal />
      <header className="fixed top-0 z-50 w-full border-b border-zinc-800/50 bg-zinc-950/60 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-4">
            {user ? (
              <div className="flex items-center gap-2">
                <Link to="/profile" className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-zinc-800/50 transition-colors">
                  <User className="w-4 h-4 text-cyan-500" />
                  <span className="text-sm font-medium hidden sm:inline-block">{user.name || user.email}</span>
                </Link>
                <button onClick={logout} className="p-2 rounded-full hover:bg-zinc-800/50 transition-colors text-red-500" title="התנתקות">
      ...
      <footer className="relative border-t border-zinc-800/50 bg-black/40 backdrop-blur-md py-6">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center space-y-3">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-x-4 gap-y-1 text-xs text-zinc-400">
