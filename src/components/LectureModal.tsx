...
export const LectureModal: React.FC<LectureModalProps> = React.memo(({ isOpen, onClose, onSave, lecture, courseId, courseNumber }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      if (lecture.fileID) {
        try {
          await storage.deleteFile(APPWRITE_CONFIG.storageBucketId, lecture.fileID);
        } catch (e) {
      ...
      console.error(err);
      setError(err.message || 'שגיאה במחיקת ההרצאה');
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  useEffect(() => {
    if (isOpen) {
      if (lecture) {
        ...
        setRightAlign(lecture.rightAlign || false);
        ...
        const loadContent = async () => {
          ...
          setNewPdfFile(null);
          try {
            const file = await storage.getFile(APPWRITE_CONFIG.storageBucketId, lecture.fileID);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const lectureName = name.trim().replace(/[^a-zA-Z0-9._-]/g, '');
      const isNameChanged = lecture && lectureName !== lecture.name?.trim().replace(/[^a-zA-Z0-9._-]/g, '');
      ...
      if (isPdf) {
        ...
        const id = `lecture-${lectureName}-${courseNum}`.toLowerCase();
        const filename = `Lecture-${lectureName}-${courseNum}.md`;
      ...
      if (lecture) {
        ...
        const docId = `lecture-${lectureName}-${courseNum}`.toLowerCase();
      ...
      console.error(err);
      setError(err.message || 'שגיאה בשמירת ההרצאה');
    } finally {
      setIsSaving(false);
    }
  };
          ...
          >
            <div className="sticky top-0 z-10 flex items-center justify-between p-6 border-b border-zinc-100 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md">
              <h2 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50">
                {lecture ? 'עריכת הרצאה' : 'הוספת הרצאה חדשה'}
              </h2>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">שם ההרצאה</label>
                <input
                  type="text"
                  required
