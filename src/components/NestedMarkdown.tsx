...
const SectionRenderer = React.memo(({ node, index }: { node: SectionNode, index: number }) => {
  ...
  if (node.level === 1) {
    containerClasses = 'p-6 sm:p-8 rounded-3xl bg-white/60 dark:bg-zinc-900/60 border border-zinc-200/50 dark:border-zinc-700/50 shadow-sm mb-8 backdrop-blur-xl scroll-mt-24';
  } else if (node.level === 2) {
    containerClasses = 'p-5 sm:p-6 rounded-2xl bg-white/50 dark:bg-zinc-800/50 border border-zinc-200/50 dark:border-zinc-600/50 shadow-sm mt-6 backdrop-blur-lg scroll-mt-24';
  } else if (node.level === 3) {
    containerClasses = 'p-4 sm:p-5 rounded-xl bg-white/40 dark:bg-zinc-700/40 border border-zinc-200/50 dark:border-zinc-500/50 shadow-sm mt-4 backdrop-blur-md scroll-mt-24';
  } else {
    containerClasses = 'p-3 sm:p-4 rounded-lg bg-white/30 dark:bg-zinc-600/30 border border-zinc-200/50 dark:border-zinc-400/50 shadow-sm mt-4 backdrop-blur-sm scroll-mt-24';
  }
        ...
        <ReactMarkdown 
          ...
          components={{
            pre({ children, node, ...props }: any) {
              // For tikz/mermaid blocks the `code` component returns its own
              ...
              if (/^language-(tikz|mermaid)$/.test(codeClass)) {
                return <>{children}</>;
              }
